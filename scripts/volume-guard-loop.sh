...
#
# Env vars:
#   SAFE_VOLUME     value to clamp back to        (default 0.30)
#   MAX_VOLUME      threshold that triggers clamp (default 0.45)
#   GUARD_INTERVAL  seconds between checks         (default 5)

SAFE_VOLUME="${SAFE_VOLUME:-0.30}"
MAX_VOLUME="${MAX_VOLUME:-0.45}"
...
# Read the default sink volume as a decimal (e.g. 0.30). Empty on failure.
read_volume() {
  # 'wpctl get-volume @DEFAULT_AUDIO_SINK@' -> "Volume: 0.30" or "Volume: 0.30 [MUTED]"
